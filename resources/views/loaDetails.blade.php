   ...
   <div class="p-4 mt-14 bg-[#f4f4f4]  h-full">
     <div class="pb-14 h-full grid grid-rows-6 grid-cols-3 gap-4">
          <div class=" p-4 col-span-2 grid  grid-cols-3 font-semibold border-solid border-2 border-[#3a4d39] rounded-md border border bg-white overflow-auto">
               ...
               <div class="p-4">
                <p class="text-xl font-thin">Status: <span class="text-xl font-thin {{ $statusColor }}">{{ $status }}</span></p>
                  
                  <p class="text-xl font-thin">Applied Quantity: {{ $loa->applied_qty }}</p>
...
@endif
          ...
          <form action="{{ route('confirm.requirement') }}" method="POST" class="col-span-2">
               ...
               <div class="col-span-3">
                  <button  
                    data-loaname = "{{ $loa->loa }}"
                    data-loaId= "{{ $loa->id }}"

                    data-loatype = "{{ $loa->type }}"
                    data-quantity = "{{ $loa->applied_qty }}"
                    data-supplier = "{{ $loa->supplier }}"
                    data-accountHolder = "{{ $loa->accountHolder }}"
                    data-accountHolderEmail = "{{ $loa->accountHolderEmail }}"
                    data-accountHolderDeptHead = "{{ $loa->accountHolderDeptHead }}"
                    data-accountHolderDeptHeadEmail= "{{ $loa->accountHolderDeptHeadEmail }}"
                    data-contractExp= "{{ $loa->contractExpirationDate }}"
                    data-deadlineCompletion= "{{ $loa->deadlineOfCompletion }}"
                  onclick="populateEditForm(this)"

                     data-modal-target="editLOAInfo"
                data-modal-toggle="editLOAInfo" type="submit" class="w-full text-white bg-gradient-to-b from-[#739072] via-[#4f6f52] to-[#3a4d39] hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg  px-5 py-2.5 text-center ">
   ...
   <div id="editLOAInfo" tabindex="-1" aria-hidden="true" class="hidden fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto h-[calc(100%-1rem)] max-h-full">
          <div class="relative w-full max-w-4xl max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              <div class="p-6 space-y-6">
                <h3 class="text-xl font-medium text-gray-900 dark:text-white">Edit Users</h3>
                <form action="{{ route('edit.loa') }}"  method="POST" >
                  @csrf

                  <input type="text" class="hidden" id="loaId" name="loaId">
          @if(session('success'))
    <div id="toast-success" class="flex items-center w-full p-4 mb-4 text-gray-500 bg-white rounded-lg shadow-sm dark:text-gray-400 dark:bg-gray-800" role="alert">
        <div class="inline-flex items-center justify-center shrink-0 w-8 h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-800 dark:text-green-200">
            <!-- Check Icon -->
...
@endif
    ...
    <div class="rounded-md bg-white text-[15px] text-gray-500 p-2 w-full ">
        ...
        <label for="" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Department Head's Email</label>
                  ...
                  <button type="button" data-modal-hide="editLOAInfo" class="w-full text-white bg-gradient-to-b from-[#f10000] via-[#ff3b3b] to-[#ff7676] hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg  px-5 py-2.5 text-center my-2">
                    <a href="">Cancel</a>
                  </button>
